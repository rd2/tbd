...
The latter is recommended for most cases - OpenStudio often sets _Ground_ as outside boundary conditions for lower floor surfaces by default. Once KIVA inputs are permanently added to an OpenStudio model, and maybe subsequently altered by hand if needed, users can safely run EnergyPlus simulations - hopefully again with TBD handling _major_, above-grade thermal bridges. Once again, it's critical to CHECK the _Alter OpenStudio model_ option, so one can subsequently tweak TBD-generated KIVA inputs.
