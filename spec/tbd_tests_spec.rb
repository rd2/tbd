...
RSpec.describe TBD do
  ...
  it "checks b40v12K" do
    translator = OpenStudio::OSVersion::VersionTranslator.new
    TBD.clean!

    file  = File.join(__dir__, "files/osms/in/b40v12k.osm")
    path  = OpenStudio::Path.new(file)
    model = translator.loadModel(path)
    expect(model).to_not be_empty
    model = model.get
    ...
    expect(TBD.logs).to_not be_empty
    expect(surfaces).to be_a Hash
    expect(surfaces.size).to eq(3782)
    expect(io).to be_a(Hash)
    expect(io).to have_key(:edges)
    expect(io[:edges].size).to eq(4699)

    file = File.join(__dir__, "files/osms/out/b40v12K.osm")
    model.save(file, true)
  end
end
