...
RSpec.describe TBD do
  ...
  it "can calculate offset for (frame & dividers)" do

    # 1m x 1m rectangle.
    vec = OpenStudio::Point3dVector.new
    vec << OpenStudio::Point3d.new( 0, 0, 1)
    vec << OpenStudio::Point3d.new( 0, 0, 0)
    vec << OpenStudio::Point3d.new( 1, 0, 0)
    vec << OpenStudio::Point3d.new( 1, 0, 1)
    ft = OpenStudio::Transformation::alignFace(vec)
    ft_inverse = ft.inverse
    flat_verts  = flatZ( (ft_inverse * vec).reverse )

    area = OpenStudio::getArea(flat_verts)
    expect(area.empty?).to be(false)
    expect(area.get).to be_within(0.0001).of(1.0)

    # adds 0.1 all around the rectangle so you end up with a 1.2m x 1.2m rectangle
    flat_offset_verts = OpenStudio::buffer(flat_verts, 0.1, 0.0001)
    expect(flat_offset_verts.empty?).to be(false)
    flat_offset_verts = flat_offset_verts.get

    offset_verts = OpenStudio::reorderULC( (ft * flat_offset_verts).reverse )
    expect(offset_verts.size).to eq(4)

    expect(offset_verts[0].x).to eq(-0.1)
    expect(offset_verts[0].y).to eq(0)
    expect(offset_verts[0].z).to eq(1.1)

    expect(offset_verts[1].x).to eq(-0.1)
    expect(offset_verts[1].y).to eq(0)
    expect(offset_verts[1].z).to eq(-0.1)

    expect(offset_verts[2].x).to eq(1.1)
    expect(offset_verts[2].y).to eq(0)
    expect(offset_verts[2].z).to eq(-0.1)

    expect(offset_verts[3].x).to eq(1.1)
    expect(offset_verts[3].y).to eq(0)
    expect(offset_verts[3].z).to eq(1.1)

    area = OpenStudio::getArea(offset_verts)
    expect(area.empty?).to be(false)
    expect(area.get).to be_within(0.0001).of(1.44)

    # triangle.
    vec = OpenStudio::Point3dVector.new
    vec << OpenStudio::Point3d.new( 0.5, 0, 1)
    vec << OpenStudio::Point3d.new( 0, 0, 0)
    vec << OpenStudio::Point3d.new( 1, 0, 0)
    ft = OpenStudio::Transformation::alignFace(vec)
    ft_inverse = ft.inverse
    ...
    area = OpenStudio::getArea(flat_verts)
    expect(area.empty?).to be(false)
    expect(area.get).to be_within(0.0001).of(0.5)
    ...
    offset_verts = OpenStudio::reorderULC( (ft * flat_offset_verts).reverse )
    expect(offset_verts.size).to eq(3)

    expect(offset_verts[0].x).to eq(0.5)
    expect(offset_verts[0].y).to eq(0)
    expect(offset_verts[0].z).to be_within(0.0001).of(1.2236) # accounts for the extra gap where the two frames meet

    expect(offset_verts[1].x).to be_within(0.0001).of(-0.1618) # accounts for the extra gap where the two frames meet
    expect(offset_verts[1].y).to eq(0)
    expect(offset_verts[1].z).to eq(-0.1)

    expect(offset_verts[2].x).to be_within(0.0001).of(1.1618) # accounts for the extra gap where the two frames meet
    expect(offset_verts[2].y).to eq(0)
    expect(offset_verts[2].z).to eq(-0.1)

    area = OpenStudio::getArea(offset_verts)
    expect(area.empty?).to be(false)
    expect(area.get).to be_within(0.0001).of(0.8760)
  end
