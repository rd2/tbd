...
RSpec.describe TBD do
  ...
  it "can uprate (ALL wall) constructions - poor (BETBG)" do
    ...
    expect(1/rsi(wall1[:lc], wall1[:f])).to be_within(TOL).of(0.642)     # R08.8
    expect(1/rsi(wall2[:lc], wall2[:f])).to be_within(TOL).of(0.477)     # R11.9
    ...
    expect(TBD.status).to eq(TBD::ERROR)
    expect(TBD.logs.size).to eq(1)
    msg = "Won't uprate '#{bulk}': (wall only) - cloning"
  ...
  it "can test 5ZoneNoHVAC (failed) uprating" do
    ...
    io, surfaces = processTBD(model, argh)
    expect(TBD.status).to eq(0)
    expect(TBD.logs.empty?).to be(true)
    ...
    expect(TBD.error?).to be(true)
    # puts TBD.logs
    # "Unable to uprate insulation layer (> 3m) of 'ASHRAE 189.1-2009 ExtWall Mass ClimateZone 5' - skipping"
    # "Unable to uprate 'ASHRAE 189.1-2009 ExtWall Mass ClimateZone 5' - skipping"
    ...
    #
    # Although this is quite a reasonable limit, why not sync with EnergyPlus?
    # BTW, there is no maximum R-value limit for OpenStudio Massless Materials.
  end
end
